...
class TestGenerateOTP:
    def test_returns_6_digit_code(self, sm):
        code = generate_otp("+447700900001", state_manager=sm)
        assert len(code) == 6
        assert code.isdigit()
    ...
    def test_stores_otp_in_state(self, sm):
        code = generate_otp("+447700900001", state_manager=sm)
        state = sm.read()
        otp = state.otp_requests.get("+447700900001")
    ...
    def test_rate_limit_allows_up_to_3(self, sm):
        generate_otp("+447700900001", state_manager=sm)
        generate_otp("+447700900001", state_manager=sm)
        generate_otp("+447700900001", state_manager=sm)
    ...
    def test_rate_limit_resets_after_window(self, sm):
        ...
        def _expire(s):
            otp = s.otp_requests["+447700900001"]
    ...
    def test_sends_whatsapp(self, sm):
        generate_otp("+447700900001", state_manager=sm)
        state = sm.read()
        assert len(state.whatsapp_log) == 1
        assert state.whatsapp_log[0].template == "otp"

    def test_without_state_manager(self):
        code = generate_otp("+447700900001")
        assert len(code) == 6
        assert code.isdigit()
...
class TestVerifyOTP:
    def test_correct_code_returns_true(self, sm):
        code = generate_otp("+447700900001", state_manager=sm)
        result = verify_otp("+447700900001", code, state_manager=sm)
        assert result is True

    def test_correct_code_removes_otp(self, sm):
        code = generate_otp("+447700900001", state_manager=sm)
        verify_otp("+447700900001", code, state_manager=sm)
        state = sm.read()
        assert "+447700900001" not in state.otp_requests

    def test_wrong_code_returns_false(self, sm):
        generate_otp("+447700900001", state_manager=sm)
        result = verify_otp("+447700900001", "000000", state_manager=sm)
        assert result is False

    def test_wrong_code_increments_attempts(self, sm):
        generate_otp("+447700900001", state_manager=sm)
        verify_otp("+447700900001", "000000", state_manager=sm)
        state = sm.read()
        assert state.otp_requests["+447700900001"].attempts == 1
    ...
    def test_expired_otp_raises_error(self, sm):
        ...
        with pytest.raises(OTPError, match="expired"):
            verify_otp("+447700900001", "123456", state_manager=sm)
    ...
    def test_no_state_manager_raises_error(self):
        with pytest.raises(OTPError, match="State manager is required"):
            verify_otp("+447700900001", "123456")
